...
void UI::CreateDesignToolBar() 
{
	...
	MenuItemImages[ITM_BUL] = "images\\Menu\\Menu_Bulb.jpg";
...
void UI::DrawGround(const GraphicsInfo& r_GfxInfo, bool selected) const
{
	string GraImage;
	if (selected)
		GraImage = "Images\\Comp\\Ground_HI.jpg";	//use image of highlighted Ground
	else
		GraImage = "Images\\Comp\\Ground.jpg";	//use image of the normal Ground

	//Draw Ground at Gfx_Info (1st corner)
	pWind->DrawImage(GraImage, r_GfxInfo.PointsList[0].x, r_GfxInfo.PointsList[0].y, COMP_WIDTH, COMP_HEIGHT);
}
...
void UI::DrawSwitch(const GraphicsInfo& r_GfxInfo, bool selected) const
{
	...
	if (selected)
		...
		SwiImage = "Images\\Comp\\Switch.jpg";	//use image of the normal Switch
